...
| App Store 수집 | RSS Feed (실패 시 에러 발생) |
| 배포 (대시보드) | Lambda (정적 페이지 서빙) |
| 배포 (API) | Lambda + API Gateway |
| 배포 (수집) | Lambda (EventBridge + API Gateway 트리거) |
...
- **FR-01.3**: RSS Feed 실패 시 에러를 발생시킨다 (fallback 없음, 명확한 에러 처리)
...
interface ReviewEvent {
  ...
  source: ReviewSource;    // 출처
  sourceReviewId: string;  // 원본 플랫폼의 리뷰/글 ID
  sourceUrl: string;       // 원본 리뷰 링크
  author: string;          // 리뷰 작성자 ID/닉네임
  rating?: number;         // 별점 (1~5, nullable — 커뮤니티 글 등 별점 없는 소스 대응)
...
- Google Play live 실패 시 에러 발생 및 source health에 BLOCKED 표시
- App Store RSS 실패 시 에러 발생 및 source health에 BLOCKED 표시
...
- **대시보드**: Lambda로 정적 페이지 서빙
- **API**: Lambda + API Gateway
- **수집 (Ingest)**: Lambda (EventBridge 자동 트리거 + API Gateway 수동 트리거)
- 모든 컴포넌트가 Lambda 기반 서버리스 구조
...
| 리스크 | 대응 |
|--------|------|
| App Store RSS 불안정 | 에러 발생 + source health BLOCKED 표시 |
| Google Play live 불안정 | 에러 발생 + source health BLOCKED 표시 |
| Bedrock 권한 없음 | keyword 기반 fallback 분류 |
