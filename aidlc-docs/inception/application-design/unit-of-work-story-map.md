...
| 역할 | Unit | 주요 작업 |
|------|------|-----------|
| Person A | Unit 2 (Ingest) | Connector, S3 저장, Bedrock 호출, 에러 처리 |
| Person B | Unit 3 (API) | REST API, 조회 로직, input validation |
| Person C | Unit 4 (Dashboard) | React UI, 대시보드, 폴링, 에러 화면 |
| Person D | Unit 5 (Infra) + Unit 6 (Prompt) | AWS 인프라 구성 + Bedrock 프롬프트 설계/튜닝 |
