...
describe("api", () => {
  it("getIssues returns parsed JSON", async () => {
    mockFetch.mockResolvedValue({
      ok: true,
      json: () => Promise.resolve([{ clusterId: "1" }]),
    });
  ...
  it("regenerateAction sends POST", async () => {
    mockFetch.mockResolvedValue({
      ok: true,
      json: () => Promise.resolve({ clusterId: "abc" }),
    });
    await api.regenerateAction("abc");
    expect(mockFetch).toHaveBeenCalledWith("/api/actions/abc", {
      method: "POST",
    });
  });
  ...
  it("throws ApiError on non-ok response", async () => {
    ...
    await expect(api.getIssues()).rejects.toMatchObject({
      status: 500,
